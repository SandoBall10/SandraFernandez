    ...
    <section id="nosotros" className="py-24 bg-transparent border-y border-gray-100/80 relative">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <RevealOnScroll className="text-center max-w-3xl mx-auto mb-16 relative z-10 bg-white/95 backdrop-blur-sm rounded-2xl px-6 py-8 border border-gray-200 shadow-sm">
          ...
          <p className="mt-3 font-mono text-[10px] sm:text-xs text-gray-700 uppercase tracking-wider font-semibold">
          ...
          <RevealOnScroll className="lg:col-span-7 space-y-8" delay={0.08}>
            <div className="bg-white p-8 rounded-2xl border border-gray-300 shadow-sm space-y-6">
              ...
              <div className="space-y-4 pt-4 border-t border-gray-300">
                <h4 className="font-sans font-bold text-sm uppercase tracking-wider text-gray-800 flex items-center gap-2">
                  <BookOpen size={16} className="text-black" />
                  Formación universitaria
                  <span className="font-mono text-[9px] text-gray-600 normal-case tracking-normal font-semibold">— Fuente SUNEDU</span>
                </h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {educacion.map((edu, idx) => (
                    <div key={idx} className="bg-gray-50 p-4 rounded-xl border border-gray-200 shadow-xs">
                      <span className="block font-sans font-bold text-sm text-black leading-snug uppercase">
                        {edu.grado}
                      </span>
                      <span className="block font-sans text-xs text-gray-700 mt-1 font-medium">
                        {edu.detalle}
                      </span>
                    </div>
                  ))}
          ...
          <RevealOnScroll className="lg:col-span-5 bg-black text-white p-8 rounded-2xl shadow-xl relative overflow-hidden" delay={0.16} direction="right">
            ...
            <div className="relative z-10 space-y-6">
              ...
              <p className="font-sans text-sm text-zinc-300 leading-relaxed">
                ...
                <span className="block font-mono text-[10px] text-zinc-400 mt-1">Fuente: MTPE — Planillas Electrónicas</span>
              </p>
              ...
              <div className="mt-6 pt-6 border-t border-zinc-800 space-y-4">
                ...
                <ul className="space-y-2">
                  {auditoria.antecedentes.map((item, idx) => (
                    <li key={idx} className="flex items-start gap-2 text-[11px] text-zinc-300 leading-snug">
                      <Award size={12} className="text-[#FFCA00] shrink-0 mt-0.5" />
                      <span>
                        <strong className="text-white">{item.tipo}</strong>
                ...
                <p className="font-mono text-[9px] text-zinc-400 leading-relaxed">
